...
export function createAssertNetworkTool(): Tool {
  return {
...
export async function handleAssertNetwork(
  args: AssertNetworkArgs
): Promise<AssertNetworkResult> {
  logger.info(
    `Assert network requested for runtime: ${args.runtime}, url=${args.url}, expectedStatus=${args.expectedStatus}`
  );
