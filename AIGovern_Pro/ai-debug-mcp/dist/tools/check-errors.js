...
export function createCheckErrorsTool() {
    return {
...
export async function handleCheckErrors(args) {
    ...
    for (const expected of args.expectedErrors) {
        const matchingEntry = deduplicatedErrors.find((e) => e.message.includes(expected) || (e.stack && e.stack.includes(expected)));
        if (matchingEntry) {
    ...
    logger.debug(`Check errors result: matched=${matched}, matchedPatterns=${matchedErrors.join(", ")}`);
    return {
        matched,
        matchedErrors,
        unmatchedErrors,
